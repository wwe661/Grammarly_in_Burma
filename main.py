...
from dictionary import load_all_dictionaries
app = FastAPI()
...
def check_text(data: TextInput):
    ...
    flags = []
    for t in tokens:
        status,source = check_token(t['token'].strip())
    ...
    for i, flag in enumerate(flags):
        ...
        if flag["status"] == "unknown":
            ...
            if correction is not None:
                ...
                status,source = check_token(message)
                color = "green"
                _ = {"start": start, "end": end, "message": message, "status": status, "source": source, "color": color}
    ...
    for i, flag in enumerate(flags):
        ...
        if flag["status"] == "unknown":
            ...
            for j in toks:
                ...
                prevtok = prevtok[-1] if len(prevtok)>1 else prevtok[0]
                msg = prevtok["message"]+ j["message"]
                st,sc = check_token(msg)
                if st == "known":
                    return None
                if j == len(toks):
                    pass
...
def check_token(token):
    if not is_burmese_token(token):
        ...
        return ("known", "Official Dictionary")
    elif token in DictB:
        return ("known", "Wiktionary")
    else:
        return ("unknown", None)
